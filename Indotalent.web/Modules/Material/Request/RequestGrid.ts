...
namespace Indotalent.Material {
    ...
    export class RequestGrid extends Serenity.EntityGrid<RequestRow, any> {
        ...
        private Status = RequestGrid.RequestStatusPending;
        ...
        protected getColumns() {
            ...
            columns.splice(2, 0, {
                ...
                sortable: true,
                format: (ctx) => {
                    const requestItem = ctx.item as RequestRow;
                    const date = new Date(requestItem.RequestDate);
                    const options: Intl.DateTimeFormatOptions = { month: 'short', year: 'numeric' };
                    return date.toLocaleDateString('en-US', options).replace(',', ' -');
                }
            });
            if (Q.Authorization.hasPermission("Administration:Approvalmanagement")) {
            ...
            if (!Q.Authorization.hasPermission("Administration:Tenant")) {
                columns = columns.filter(x => x.field != RequestRow.Fields.TenantName && x.field != RequestRow.Fields.Id);
            }
        ...
        protected onClick(e: JQueryEventObject, row: number, cell: number) {
            ...
            if (target.hasClass('inline-action')) {
                ...
                if (target.hasClass('Approve-action')) {
                    ...
                    var CurrentAction = RequestGrid.Approved;
                ...
                if (target.hasClass('Reject-action')) {
                    ...
                    var CurrentAction = RequestGrid.Rejected;
        ...
        protected ProcessActionRequest(CurrentAction: string, row: number) {
            ...
            if (CurrentAction == RequestGrid.Approved) {

                item.RequestStatus = RequestGrid.RequestStatusApproved;


                Indotalent.Material.MaterialRequestService.Update({
            ...
            if (CurrentAction == RequestGrid.Rejected) {

                item.RequestStatus = RequestGrid.RequestStatusRejected;
        ...
        //protected ProcessReviewRequest(item: RequestRow) {
        //    var list = item.ItemList;
        //    var dialog = new RequestDialog(item.Id);
        //    dialog.loadEntityAndOpenDialog(item);
           
        //   /* new Material.RequestDialog(item.Id).dialogOpen();*/
        //    dialog.set_readOnly(true);
        //}
        protected ProcessReviewRequest(item: RequestDetailRow) {
